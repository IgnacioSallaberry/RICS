...
while q<61:
    ...
    gamma = 0.3536        #gamma factor de la 3DG
               #Numero total de particulas en la PSF
    
    

    
    
    def Binding_H_line (x,N,Ab,t_bind):
        box_size = 256
        roi = 128
        tp = 5e-6             #seg    
        tl = box_size * tp    #seg
...
#        return ( (gamma/N) + Bind) 
    ...
    if mostrar_imagenes:
        fig = plt.figure()
        ax = fig.add_subplot(111, projection='3d')
        ax.plot_trisurf(X,Y, Binding(X_DATA,0.05,1.75,0.005),cmap='viridis', edgecolor='none')
    ...
    fit_params, cov_mat = curve_fit(Binding,X_DATA, G, p0=(0.05,1.75,0.00497))
    ...
    if mostrar_imagenes:
        fig = plt.figure()
        ax = fig.add_subplot(111, projection='3d')
        ax.plot_trisurf(X,Y, Binding(X_DATA,fit_params[0],fit_params[1],fit_params[2]),cmap='viridis', edgecolor='none')
        ax.set_xlabel(r'pixel shift $\xi$', fontsize=15)
        ax.set_ylabel(r'pixel shift $\psi$',fontsize=15)
        ax.set_zlabel(r'G($\xi$,$\psi$)',fontsize=15)
    ...
    def chi2(X_DATA,N,Ab,t_bind):
        return np.sum((Binding(X_DATA,N,Ab,t_bind) - G)**2)/3969
    
    
#    print(chi2(X_DATA,fit_params[0],fit_params[1]))   ## si quiero ver el valor del chi2 ajustado
    
    S_2_ajustado_por_python_Binding.append(chi2(X_DATA,fit_params[0],fit_params[1],fit_params[2]))
    
    q+=1
    ##### para guardar los datos

with open('S2 calculado de ajustar con python __ Ajuste de proceso de {} por modelo de Binding PURO.txt'.format(proceso),'w') as f:
    for valor in S_2_ajustado_por_python_Binding:
